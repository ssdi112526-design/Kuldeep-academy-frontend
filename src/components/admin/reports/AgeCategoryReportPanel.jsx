...
export default function AgeCategoryReportPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await reportsService.ageCategories(filters);
        if (!cancelled) setData(res.data.data);
      } catch (err) {
        if (!cancelled) {
          toast.error(getApiErrorMessage(err, 'Failed to load age categories'));
          setData(null);
        }
      } finally {
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-ink">Age category report</h2>
          <p className="mt-1 text-sm text-muted">Players grouped by age category.</p>
        </div>
        <ReportExportBar
          reportKey="age-category"
          filters={filters}
          canExport={canExport}
          canPrint={canPrint}
          toast={toast}
