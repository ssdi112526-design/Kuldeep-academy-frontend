...
export default function PlayerReportAttendance({ studentId }) {
  ...
  const historyRows = useMemo(() => {
    const days = grid?.days || [];
    return [...days]
      .filter((row) => row.morning?.status || row.evening?.status)
      .reverse()
      .map((row) => ({
        ...row,
  ...
  const summaryStats = useMemo(() => {
    const combined = grid?.totals?.combined || {};
    const present = combined.present || 0;
    const absent = combined.absent || 0;
    const leave = combined.leave || 0;
    const medicalLeave = combined.medical_leave || 0;
    const competitionLeave = combined.competition_leave || 0;
    const totalSessions = present + absent + leave + medicalLeave + competitionLeave;
  ...
  const statusLabel = (status) => STATUSES.find((s) => s.key === status)?.label || '—';

  return (
    <div className="space-y-4">
      <section className="rounded-xl border border-slate-100 bg-white p-4 sm:p-5">
        <h3 className="text-base font-bold text-ink">Attendance Summary</h3>
        {loading ? (
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-7">
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-3">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-muted">Total Days</p>
              <p className="mt-1 text-xl font-bold tabular-nums text-ink">{summaryStats.totalDays}</p>
            </div>
            {STATUSES.map((status) => (
            ...
            <div className="rounded-xl border border-slate-100 bg-white px-3 py-3 sm:col-span-2 lg:col-span-1">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-muted">Attendance %</p>
              <p className="mt-1 text-xl font-bold tabular-nums text-ink">{summaryStats.attendancePct}%</p>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="rounded-xl border border-slate-100 bg-white p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-base font-bold text-ink">Attendance Records ({monthLabel(year, month)})</h3>
            ...
            <table className="min-w-[480px] w-full border-collapse text-left text-sm">
              <thead>
                <tr className="bg-slate-50 text-[11px] font-semibold uppercase tracking-wide text-muted">
                  <th className="border border-slate-100 px-3 py-2">Date</th>
                  <th className="border border-slate-100 px-3 py-2">Morning</th>
                  <th className="border border-slate-100 px-3 py-2">Evening</th>
                </tr>
              </thead>
              <tbody>
                {historyRows.map((row) => (
                  <tr key={row.date}>
                    <td className="border border-slate-100 px-3 py-2 font-medium text-ink">{row.dateLabel}</td>
      ...
      <section className="rounded-xl border border-slate-100 bg-white p-4 sm:p-5">
        <h3 className="text-base font-bold text-ink">Session Grid ({monthLabel(year, month)})</h3>
