...
export default function PlayerCategoryReportPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await reportsService.playerCategories(filters);
        if (!cancelled) setData(res.data.data);
      } catch (err) {
        if (!cancelled) {
          toast.error(getApiErrorMessage(err, 'Failed to load player categories'));
          setData(null);
        }
      } finally {
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-ink">Player category report</h2>
          <p className="mt-1 text-sm text-muted">Players grouped by category / membership.</p>
        </div>
        <ReportExportBar
          reportKey="player-category"
          filters={filters}
          canExport={canExport}
          canPrint={canPrint}
          toast={toast}
                ...
                {isOpen && (
                  <ul className="border-t border-slate-50 px-4 py-2 text-sm">
                    {(g.players || []).map((p) => (
                      <li
                        key={p.id || p._id}
                        className="flex justify-between gap-2 border-b border-slate-50 py-2 last:border-0"
                      >
